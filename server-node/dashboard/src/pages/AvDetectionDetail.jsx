...
export default function AvDetectionDetail() {
  ...
  if (!detection) return <div className={styles.error}>Detection not found</div>;
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <Link to="/av/detections" className={styles.back}>← Detections</Link>
        <h1 className={styles.title}>{detection.detection_name || 'Detection'}</h1>
        <span className={`${styles.badge} ${severityClass(detection.severity)}`}>{detection.severity}</span>
        <span className={styles.badge}>{detection.detection_type}</span>
      </header>
