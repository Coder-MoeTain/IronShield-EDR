  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <h1 className={styles.title}><span className={styles.titleIcon}>🛡</span> Quarantine Manager</h1>
      </header>
                ...
                <tr key={q.id}>
                  ...
                  <td>
                    {q.status === 'quarantined' && (
                      <>
                        <button className={styles.quickLink} style={{ marginRight: 8 }} onClick={() => handleRestore(q.id)}>Restore</button>
                        <button className={styles.quickLink} onClick={() => handleDelete(q.id)}>Delete</button>
                      </>
                    )}
                  </td>
                </tr>
